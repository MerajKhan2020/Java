...
public class BatchModeGame {
    ...
    public static void play(Punter punter, List<Die> dice) {
        ...
        while (roundCount != numberOfGames && punter.balanceExceedsLimitBy(stdBet)) {
            ...
            winnings = Round.play(punter, dice, pick, stdBet);
            roundCount++;
            ...
            if (winnings > 0) {
                System.out.println(String.format("\n%s won %d, balance now %d\n\n",
                        punter.getName(), winnings, punter.finalBalanaceAfterWinning(winnings)));
                totalWins++;
            }
            else {
                System.out.println(String.format("\n%s lost %d, balance now %d\n\n",
                        punter.getName(), stdBet, punter.finalBalanceAfterLosingBet(stdBet)));
                totalLoses++;
            }
        }
        ...
        System.out.println("Total number of loses "+ totalLoses);
		...
		System.out.println("The winning ratio is " + winningRatio);
